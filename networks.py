...
class LGGNet(nn.Module):
    ...
    def __init__(self, num_classes, input_size, sampling_rate, num_T,
        ...
        #                                        (1, int(self.window[4] * sampling_rate)),
        #                                        self.pool, pool_step_rate)
        # self.Tception6 = self.temporal_learner(input_size[0], num_T,
        #                                        (1, int(self.window[5] * sampling_rate)),
        #                                        self.pool, pool_step_rate)
        # self.Tception7 = self.temporal_learner(input_size[0], num_T,
        #                                        (1, int(self.window[6] * sampling_rate)),
        #                                        self.pool, pool_step_rate)
        # self.Tception8 = self.temporal_learner(input_size[0], num_T,
        #                                        (1, int(self.window[7] * sampling_rate)),
        #                                        self.pool, pool_step_rate)
    ...
    def get_size_temporal(self, input_size):
        ...
        # out = torch.cat((out, z), dim=-1)
        # z = self.Tception6(data)
        # out = torch.cat((out, z), dim=-1)
        # z = self.Tception7(data)
        # out = torch.cat((out, z), dim=-1)
        # z = self.Tception8(data)
        # out = torch.cat((out, z), dim=-1)
