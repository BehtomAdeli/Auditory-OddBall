# This is the new edited networks script
import torch
import math
...
class LGGNetnew(nn.Module):
    ...
    def __init__(self, num_classes, input_size, sampling_rate, num_T,
        ...
        self.Tception1 = self.temporal_learner(input_size[0], num_T,
                                               (3, 1, int(self.window[0] * sampling_rate)),
                                               self.pool, pool_step_rate)
        self.Tception2 = self.temporal_learner(input_size[0], num_T,
                                               (3, 1, int(self.window[1] * sampling_rate)),
                                               self.pool, pool_step_rate)
        self.Tception3 = self.temporal_learner(input_size[0], num_T,
                                               (3, 1, int(self.window[2] * sampling_rate)),
                                               self.pool, pool_step_rate)
        # self.Tception4 = self.temporal_learner(input_size[0], num_T,
        #                                        (3, 1, int(self.window[3] * sampling_rate)),
        #                                        self.pool, pool_step_rate)
        # self.Tception5 = self.temporal_learner(input_size[0], num_T,
        #                                        (3, 1, int(self.window[4] * sampling_rate)),
        #                                        self.pool, pool_step_rate)
        self.BN_t = nn.BatchNorm3d(num_T)
        self.BN_t_ = nn.BatchNorm3d(num_T)
        self.OneXOneConv = nn.Sequential(
            nn.Conv3d(num_T, num_T, kernel_size=(1, 1, 1), stride=(1, 1, 1)),
            nn.LeakyReLU(),
            nn.AvgPool3d((1, 1, 2)))
        self.OneXOneConv2 = nn.Sequential(
            nn.Conv3d(num_T, num_T, kernel_size=(1, 1, 3), stride=(1, 1, 1)),
            nn.LeakyReLU(),
            nn.AvgPool3d((1, 1, 2)))
    ...
    def get_size_temporal(self, input_size):
        ...
        #3D input
        data = torch.ones((1, input_size[0],3, input_size[1], int(input_size[2])))
